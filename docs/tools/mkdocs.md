...
Для установки используется пакетный менеджер `pip`{: title="Python Installs Packages"}, который поставляется вместе с Python.
...
Встроенные плагины доступны сразу после установки MkDocs. Достаточно подключить их в `mkdocs.yml`{: title="конфигурационный файл проекта"}.
...
Для подключения активируйте плагин в `mkdocs.yml`{: title="конфигурационный файл проекта"}:
...
2. Добавьте список тегов в `mkdocs.yml`{: title="конфигурационный файл проекта"}:
```go title="mkdocs.yml"
plugins:
  - search
  - tags:
...
Offline — встроенный плагин для создания документации, работающей без интернета. Позволяет пользователям открывать сайт локально.
...
Внешние плагины требуют предварительной установки через `pip`{: title="Python Installs Packages"}. Затем они активируются в `mkdocs.yml`{: title="конфигурационный файл проекта"}.

#### GLightbox

GLightbox – внешний плагин для создания масштабируемой галереи изображений из картинок, добавленных в статью.

1. Установите GLightbox с помощью `pip`{: title="Python Installs Packages"}:
```
pip install mkdocs-glightbox
```
2. Активируйте плагин в `mkdocs.yml`{: title="конфигурационный файл проекта"}:
...
PUML – внешний плагин для встраивания диаграмм PlantUML в документацию. Диаграммы описываются текстом прямо в md-файлах, а при сборке проекта заменяются на готовые SVG–изображения.

1\. Установите плагин с помощью `pip`{: title="Python Installs Packages"}:
...
Для публикации этого проекта используется GitHub Pages. Развёртывание выполняется через встроенную команду MkDocs, которая использует скрипт [ghp-import](https://github.com/c-w/ghp-import).

### Ручная публикация

1. Откройте командную строку в рабочей директории проекта.
...
Для изменения оформления сайта используется файл со стилями `extra.css`. Он подключается в `mkdocs.yml`{: title="конфигурационный файл проекта"} и переопределяет стандартные стили темы MkDocs Material.

!!! info "Справка"
    CSS — язык стилей, который определяет внешний вид веб-страниц. CSS описывает, как элементы HTML должны отображаться на экране.
...
3. Подключите файл `extra.css` в `mkdocs.yml`{: title="конфигурационный файл проекта"}:
```go title="mkdocs.yml"
extra_css:
...
В файле `extra.css` переопределяются CSS-переменные для стандартных светлой и тёмной цветовых схем:
...
```
2. Подключите созданную цветовую схему вместо стандартных в `mkdocs.yml`{: title="конфигурационный файл проекта"}:
```go title="mkdocs.yml"
theme:
…
...
!!! warning "Важно!"
    При подключении пользовательской схемы стандартную `default` неободимо отключить. Если оставить обе схемы, переключение между ними будет работать некорректно, так как MkDocs не различает «основную» и «ночную» схемы. Отключение выполняется с помощью комментария `#`:
    ```go title="mkdocs.yml"
    theme:
    …
      palette: # Настройки тем
    ...
    #      accent: blue # Акцентный цвет
        ...
        - scheme: slate
          ...
          accent: blue
    ```
    Если в проекте нужна одна цветовая схема, в `mkdocs.yml`{: title="конфигурационный файл проекта"} оставьте настройки только одной цветовой схемы.
...
Плагин [mkdocs_puml](#h4#puml) по умолчанию генерирует две версии диаграммы: для светлой схемы и для тёмной темы. Это работает корректно, пока используются стандартные имена схем.
...
Расширение `pymdownx.snippets` подключается в `mkdocs.yml`{: title="конфигурационный файл проекта"}:
