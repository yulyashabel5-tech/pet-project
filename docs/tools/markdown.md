...
Текст, размеченный с помощью маркдауна, легко читать в исходном виде и править в любом редакторе. При необходимости он легко преобразуется в другие форматы, например, в `HTML`{: title="Hypertext Markup Language"} или `PDF`{: title="Portable Document Format"}.
